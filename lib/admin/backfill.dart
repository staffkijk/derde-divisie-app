...
int _toInt(dynamic v) {
  ...
  final s =
      (v ?? '').toString().trim().replaceAll('\u2212', '-'); // U+2212 -> '-'
...
Future<void> backfillDvDt({required String divisie, bool dryRun = true}) async {
  ...
  final q1 =
      db.collection('matches').where('competitie', isEqualTo: divisie).get();
  final q2 =
      db.collection('matches').where('division', isEqualTo: divCode).get();
  ...
  for (final m in matches.values) {
    final homeTeam =
        _str(m['homeTeam'] ?? m['thuis'] ?? m['home'] ?? m['teamHome']);
    final awayTeam =
        _str(m['awayTeam'] ?? m['uit'] ?? m['away'] ?? m['teamAway']);

    // Scores (tolerant op veldnamen)
    final hg =
        _toInt(m['homeGoals'] ?? m['hg'] ?? m['thuisGoals'] ?? m['scoreHome']);
    final ag =
        _toInt(m['awayGoals'] ?? m['ag'] ?? m['uitGoals'] ?? m['scoreAway']);
  ...
  // ignore: avoid_print
  print(
      '[BACKFILL] divisie="$divisie" updates=$updates missing=$missing dryRun=$dryRun');
}
