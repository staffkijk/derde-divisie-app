...
class WedstrijdenPouleDdbScreen extends StatefulWidget {
  ...
  @override
  State<WedstrijdenPouleDdbScreen> createState() =>
      _WedstrijdenPouleDdbScreenState();
}
...
class _WedstrijdenPouleDdbScreenState extends State<WedstrijdenPouleDdbScreen> {
  ...
  /// in de huidige speelronde (poule_voorspellingen = DDB).
  void _listenPouleVoorspellingen() {
    ...
    // whereIn max 10: per ronde zijn er 9 wedstrijden → ok.
    _predictionsSub = FirebaseFirestore.instance
        .collection('poule_voorspellingen')
  ...
  Future<void> _opslaanVoorspelling(Wedstrijd w, String thuis, String uit) async {
    ...
    await FirebaseFirestore.instance
        .collection('poule_voorspellingen') // DDB
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Voorspel Derde Divisie B (Poule)')),
      body: SafeArea(
        child: Column(
