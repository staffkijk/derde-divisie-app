...
const FULL_RUN_ID = 'synthetic-2026-2027-v1';
...
function loadValidatedReport() {
  const script = path.join(__dirname, 'synthetic_2026_2027_dry_run.js');
...
function selectDataset(report) {
  ...
  if (!report.validation.valid || !report.validation.inheritedPilot?.valid) fail('De inherited volledige dry-run is niet valide.');
...
async function verifyInheritedPilot(dataset) {
  const errors = [];
  const manifestPath = `synthetic_runs/${PILOT_RUN_ID}`;
  const manifestSnap = await db.doc(manifestPath).get();
  if (!manifestSnap.exists) return { valid: false, errors: ['pilotmanifest ontbreekt'], manifestPath };
  const manifest = manifestSnap.data();
  if (manifest.status !== 'verified') errors.push(`pilotstatus=${manifest.status}`);
  if (manifest.configHash !== PILOT_CONFIG_HASH) errors.push(`pilot configHash=${manifest.configHash}`);
  if (manifest.validatedConfigHash !== undefined && manifest.validatedConfigHash !== VALIDATED_CONFIG_HASH) errors.push(`pilot validatedConfigHash=${manifest.validatedConfigHash}`);
  if (manifest.syntheticRunId !== PILOT_RUN_ID) errors.push('pilotmanifest syntheticRunId wijkt af');
  const fixed = [
    ['syn_2026_2027_001', 'Milan de Boer', 'milan92'], ['syn_2026_2027_002', 'Sophie Vermeer', 'sofievermeer'],
    ['syn_2026_2027_003', 'Jeroen Bakker', 'jbakker83'], ['syn_2026_2027_004', 'Lisa van Dijk', 'lisavd'],
    ['syn_2026_2027_005', 'Tom van Dijk', 'tomdijk'],
  ];
  const expectedPaths = [];
  for (const [uid, displayName, username] of fixed) {
    const userPath = `users/${uid}`; const indexPath = `usernames/${normalize(username)}`;
    expectedPaths.push(userPath, indexPath);
    const [userSnap, indexSnap] = await db.getAll(db.doc(userPath), db.doc(indexPath));
    if (!userSnap.exists) errors.push(`${userPath} ontbreekt`); else { const user = userSnap.data(); if (user.displayName !== displayName || user.username !== username || user.syntheticRunId !== PILOT_RUN_ID) errors.push(`${userPath} identity/marker wijkt af`); }
    if (!indexSnap.exists) errors.push(`${indexPath} ontbreekt`); else { const index = indexSnap.data(); if (index.uid !== uid || index.username !== username || index.syntheticRunId !== PILOT_RUN_ID) errors.push(`${indexPath} ownership/marker wijkt af`); }
  }
  const pilotPredictions = dataset.predictions.filter((item) => PILOT_IDS.has(item.syntheticId) && item.round === 1);
  for (const item of pilotPredictions) {
    const docPath = `seasons/${SEASON}/predictions/${item.syntheticId}_${item.matchId}`; expectedPaths.push(docPath);
    const snap = await db.doc(docPath).get(); const data = snap.exists ? snap.data() : null;
    if (!data) errors.push(`${docPath} ontbreekt`); else if (data.syntheticRunId !== PILOT_RUN_ID || data.gebruikerId !== item.syntheticId || data.matchId !== item.matchId || data.homeScore !== item.homeGoals || data.awayScore !== item.awayGoals || data.division !== item.division || data.predictionProfile !== item.profile) errors.push(`${docPath} inhoud/marker wijkt af`);
  }
  const pilotFinals = dataset.finals.filter((item) => PILOT_IDS.has(item.syntheticId));
  for (const item of pilotFinals) {
    const docPath = `eindstand_voorspellingen/${item.syntheticId}_${item.division}`; expectedPaths.push(docPath);
    const snap = await db.doc(docPath).get(); const data = snap.exists ? snap.data() : null;
    if (!data) errors.push(`${docPath} ontbreekt`); else if (data.syntheticRunId !== PILOT_RUN_ID || data.gebruikerId !== item.syntheticId || data.divisie !== item.division || data.predictionProfile !== item.profile || JSON.stringify(data.voorspelling) !== JSON.stringify(item.ranking)) errors.push(`${docPath} inhoud/marker wijkt af`);
  }
  const registeredDataPaths = (manifest.allDocumentPaths || []).filter((item) => item !== manifestPath);
  if (expectedPaths.length !== 70 || registeredDataPaths.length !== 70 || manifest.allDocumentPaths?.length !== 71) errors.push(`pilotpadaantallen verwacht=70/71 werkelijk=${registeredDataPaths.length}/${manifest.allDocumentPaths?.length}`);
  if (JSON.stringify([...expectedPaths].sort()) !== JSON.stringify([...registeredDataPaths].sort())) errors.push('pilotmanifestpaden wijken af van effectieve pilotdataset');
  return { valid: errors.length === 0, errors, manifestPath, status: manifest.status, configHash: manifest.configHash, validatedConfigHash: manifest.validatedConfigHash, inheritedUsers: fixed.map(([syntheticId, displayName, username]) => ({ syntheticId, displayName, username })), counts: { users: 5, usernames: 5, predictions: pilotPredictions.length, finalStandings: pilotFinals.length, dataDocuments: expectedPaths.length, manifestPaths: manifest.allDocumentPaths?.length } };
}
...
async function verifyGlobalTotals(dataset, protectedBaseline) {
  const errors = [];
  const fullIds = dataset.players.map((player) => player.syntheticId);
  ...
  if (userSnapshots.filter((snapshot) => snapshot.exists).length !== 50) errors.push('synthetic users totaal niet 50');
  if (usernameSnapshots.filter((snapshot) => snapshot.exists).length !== 50) errors.push('username-indexen totaal niet 50');
  for (const snapshot of userSnapshots) if (snapshot.exists) { const expectedRun = PILOT_IDS.has(snapshot.id) ? PILOT_RUN_ID : FULL_RUN_ID; if (snapshot.data().syntheticRunId !== expectedRun) errors.push(`${snapshot.ref.path}: marker wijkt af`); }
  ...
  if (seasonPredictions.size !== 15912) errors.push(`predictions totaal=${seasonPredictions.size}`);
  ...
  if (syntheticFinals.length !== 52) errors.push(`eindstanden totaal=${syntheticFinals.length}`);
  const pools = await db.collection('poules').where('syntheticRunId', '==', FULL_RUN_ID).get();
  ...
  let userPoolIndexes = 0; for (const id of fullIds) userPoolIndexes += (await db.doc(`users/${id}`).collection('poules').where('syntheticRunId', '==', FULL_RUN_ID).get()).size;
  ...
  return { users: 50, usernameIndexes: 50, predictions: seasonPredictions.size, finalStandings: syntheticFinals.length, pools: pools.size, poolParticipants: participants, userPoolIndexes, oldFakeStubs: after.oldFakeCount, rootPredictionsWritten: 0, authenticationWrites: 0 };
}
...
async function main() {
  ...
  const datasetTotalsAfterApply = { users: 50, usernameIndexes: 50, predictions: dataset.predictions.length, finalStandings: 52, poolsAndMembershipIndexes: 82, pilotManifest: 1, mainManifest: 1 };
  ...
  console.log(JSON.stringify({
    ...
    inheritedPilotManifest: pilotVerification.manifestPath,
    inheritedUsers: pilotVerification.inheritedUsers.length,
    existingPilotDocuments: 71,
    planned,
    ...
    poolsIncluded: dataset.pools.map((pool) => pool.id),
    collisionChecks: { inheritedPilotReservationsAccepted: 5, inheritedIdentityMutations: 0, newUserCollisionScope: '006-050', displayNameCollisions: dataset.report.validation.identities.currentDisplayNameCollisions, usernameCollisions: dataset.report.validation.identities.currentUsernameCollisions, existingTargetPaths: validation.errors.filter((error) => error.startsWith('Doelpaden bestaan al:') || error.startsWith('Bestaande manifestchunkpaden:')) },
    validation,
  }, null, 2));
  ...
  await db.doc(manifestPath).create({
    ...
    pilotRunId: PILOT_RUN_ID, createdAt: FieldValue.serverTimestamp(), verifiedAt: FieldValue.serverTimestamp(),
