...
const RUN_ID = "synthetic-2026-2027-v2";
...
const IDENTITY_SEEDS = [
  ["Daan Verhoeven", "daanv87"],
  ["Mark de Graaf", "markdg"],
  ["Jesse Koster", "jessek"],
  ["Sven Mulder", "svenmulder"],
  ["Robin Visser", "robinv"],
  ["Bart Jansen", "bartj82"],
  ["Wesley de Jong", "wesleydej"],
  ["Luuk Hendriks", "luukh"],
  ["Mike Peters", "mikep90"],
  ["Dennis Smit", "denniss"],
  ["Kevin van Loon", "kevinvl"],
  ["Jordy Bos", "jordyb"],
  ["Patrick Kuiper", "patrickk"],
  ["Niek Schouten", "nieks"],
  ["Remco Blom", "remcob"],
  ["Frank de Wit", "frankdw"],
  ["Sander Meijer", "sandermeijer"],
  ["Rik van Dam", "rikvd"],
  ["Maarten Vos", "maartenv"],
  ["Johan Dekker", "johand"],
  ["Gijs Brouwer", "gijsb"],
  ["Erik Timmer", "erikt"],
  ["Wim Post", "wimpost"],
  ["Joep van Beek", "joepvb"],
  ["Marco de Ruiter", "marcodr"],
  ["Bas Kok", "baskok"],
  ["Thom Jacobs", "thomj"],
  ["Ramon Willems", "ramonw"],
  ["Pim Scholten", "pims"],
  ["Jacco Peeters", "jacco_p"],
  ["Hugo Groen", "hugog"],
  ["Ivo Martens", "ivom"],
  ["Steef van Leeuwen", "steefvl"],
  ["Ralf Hoekstra", "ralfh"],
  ["Cor de Groot", "cordg"],
  ["Jaap Mol", "jaapmol"],
  ["Max Smits", "maxs26"],
  ["Teun van den Berg", "teunvdb"],
  ["Arno Bakker", "arnob"],
  ["Paul Vermeer", "paulv"],
  ["Nick Prins", "nickprins"],
  ["Menno Dijkstra", "mennod"],
  ["Tijn de Bruin", "tijndb"],
  ["Ruud Kramer", "ruudk"],
  ["Sam de Boer", "samdb"],
  ["Fleur Koster", "fleur26"],
  ["Nora Visser", "norav"],
  ["Anne Mulder", "annem"],
  ["Sophie Groen", "sophieg"],
  ["Maaike Bos", "maaikeb"],
];
...
function buildPlayers(strengths, inputs) {
  ...
  const inheritedIds = new Set(inputs.liveUsers.filter((user) => user.syntheticRunId === RUN_ID).map((user) => user.id));
  const inheritedUsernameKeys = new Set(inputs.usernames.filter((item) => item.syntheticRunId === RUN_ID).map((item) => item.id));
  ...
  return assignments.map((competitions, index) => {
    const syntheticId = `syn_2026_2027_${String(index + 51).padStart(3, "0")}`;
...
function buildPools(players, strengths) {
  return { pools: [], validation: { valid: true, poolCount: 0, participantDocuments: 0, userPoolIndexes: 0, errors: [] } };
}
